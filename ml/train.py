...
df = pd.read_csv('ml/Preprocessed.csv')
print(df)
...
df = make_dfs(df)
df = df.drop(df.index[113]) #REMOVING AN OUTLIER
print(df[df['Amount']==0.625]['id'])
...
def kfold(X_train,y_train, nfolds, model):
    ...
    scores = cross_val_score(predictor, X_train, y_train, scoring='r2', cv=folds)
...
#print(df_pred['Diff%'].describe())
#print(df_pred['Difference'].describe())

print(r2_score(df_pred['Y'],df_pred['Pred']))

for i in range(len(df)):
    if (df.iloc[i].name==237):
        print(df.iloc[i])
        #print(i)
        pass
...
def best_model(X_train,y_train):
    ...
    for model in models:
        ...
        scores = loocv(X_train, y_train, model)
    ...
    for i in range(3):
        ...
        print("Kfold: "+str(mean(scores)))
        scores = loocv(X_poly, y_train, lin_reg)
        print("LOOCV: "+str(mean(scores)))
#best_model(X_feat, Y)
